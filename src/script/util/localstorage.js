...
/** 对外暴露键值 **/
var mainKey = 'ls';
var updateKey = mainKey + '_update';
var metaKEY = '__' + mainKey + '_meta';
...
(function (targetExport) {
    ...
    targetExport.prototype = {
         ...
         * @returns {boolean}
         */
        quickSet    : function (key, value) {
            if (!key) {
                return false;
            }
            try {
                var val = JSON.stringify(value);
                _storageService[key] = val;
                // If userData is used as the storage engine, additional
                if (_storageElm) {
                    _storageElm.setAttribute(key, val);
                    _storageElm.save(key);
                }
            } catch (E7) { /* probably cache is full, nothing is saved this way*/
            }
        },
        quickGet    : function (key) {
            _checkKey(key);
            if (key in _storageService) {
                return _storageService[key];
            }
            return null;
        },
        quickRemove : function (key) {
            _checkKey(key);
            if (key in _storageService) {
                delete _storageService[key];
                return true;
            }
            return false;
        },
        quickIndex  : function () {
            var index = [],
                i;
            for (i in _storageService) {
                if (_storageService.hasOwnProperty) {
                    if (_storageService.hasOwnProperty(i) && i != metaKEY) {
                        index.push(i);
                    }
                } else {
                    if (_storageService[i] && i != metaKEY) {
                        index.push(i);
                    }
                }
            }
